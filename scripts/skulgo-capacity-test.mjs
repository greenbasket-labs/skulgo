...
async function login(u){
 ...
 const d=await r.json().catch(()=>({})); if(!r.ok)throw Error(`Login failed for ${u.email}: HTTP ${r.status} ${d.error||""}`); if(!/^\d{4,6}$/.test(String(u.pin||"")))throw Error(`Test user ${u.email} requires a 4-6 digit pin in TEST_USERS`);
 ...
 const c=cookies(r);
 const s=await fetch(target+"/api/workspaces/select",{method:"POST",headers:{"content-type":"application/json",...(c?{cookie:c}:{})},body:JSON.stringify({membershipId:w.membershipId,pin:u.pin})});
